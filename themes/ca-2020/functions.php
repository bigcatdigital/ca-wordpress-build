...
//Custom: add Project CPT custom fields to API response data 
function bc_rest_prepare_project($data, $post, $request) {
	$_data = $data->data;
	//Just add the project categories to the JSON response
	$project_categories = get_field('project-categories', $post->ID);
	$project_location = get_field('project-location', $post->ID);
	$project_img = get_field('project-square-image', $post->ID);
	$data->data['project-categories'] = $project_categories;
	$data->data['project-location'] = $project_location;
	$data->data['project-img'] = $project_img;
	return $data;
}
add_filter('rest_prepare_project', 'bc_rest_prepare_project', 10, 3);
?>